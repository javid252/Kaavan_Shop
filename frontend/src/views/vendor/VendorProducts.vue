<template>
  <div class="vendor-products">
    <div class="page-head">
      <h1>محصولات من</h1>
      <router-link to="/vendor/products/new" class="btn btn-primary btn-sm">+ محصول جدید</router-link>
    </div>
        ...
        <tbody>
          <tr v-for="p in products" :key="p.id">
            ...
            <td class="actions-cell">
              <router-link :to="`/vendor/products/${p.slug}/edit`" class="btn btn-outline btn-sm">ویرایش</router-link>
              <button class="btn btn-danger btn-sm" @click="remove(p)">حذف</button>
            </td>
          </tr>
        </tbody>
      </table>
      <p v-if="!loading && products.length === 0" class="text-muted empty-row">هنوز محصولی ثبت نکرده‌اید.</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async fetchProducts() {
      this.loading = true;
      try {
        const { data } = await api.get("/products/", { params: { mine: 1, search: this.search, page_size: 100 } });
