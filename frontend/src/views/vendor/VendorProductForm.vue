<template>
  <div class="vendor-product-form">
    <div class="page-head">
      ...
      <router-link to="/vendor/products" class="btn btn-outline btn-sm">بازگشت به لیست</router-link>
    </div>
    ...
    <form v-else class="card form-card" @submit.prevent="submit">
      ...
      <div class="checkbox-row">
        <label><input v-model="form.is_active" type="checkbox" /> فعال / قابل نمایش در فروشگاه</label>
      </div>
  ...
  data() {
    return {
      ...
      form: {
        ...
        discount_price: null, stock: 0, is_active: true,
      },
      productId: null,
      images: [],
      pendingFiles: [],
    ...
    async submit() {
      ...
      try {
        if (this.isEdit) {
          ...
          this.$router.replace(`/vendor/products/${data.slug}/edit`);
          return;
        }
